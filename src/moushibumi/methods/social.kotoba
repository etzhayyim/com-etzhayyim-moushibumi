(ns moushibumi.methods.social
  "social.cljc — 申文 — Citizen Democratic-Participation Concierge DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from moushibumi's own
...
(def DISCLAIMER
  (str "【観測ミラー / accountability map — NOT a verdict, NOT advice, 非断定】 "
       "申文 — Citizen Democratic-Participation Concierge が既知の観測から編んだ事実の要約です。"))
...
(defn draft-observation-post
  "A dry-run observation post over moushibumi's own domain (kotoba-native citizen democratic-participation concierge — the citizen's VOICE INTO the state, sibling to toritsugi (government procedures)…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "moushibumi R0: live social posting is Council Lv6+ + operator + "
